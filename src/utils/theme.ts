...
export function getTheme(themeName: ThemeName): Theme {
  ...
  const accent = (
    process.env.CLAUDE_CODE_ACCENT_COLOR ?? "default"
  ).toLowerCase();
  if (accent === "default") return baseTheme;

  const isAnsi = baseTheme.claude.startsWith("ansi:");
  const palette = isAnsi
    ? {
        orange: {
    ...
    : {
        orange: {
          ...
          shimmer: "rgb(245,149,117)",
          merged: "rgb(215,119,87)",
        },
        green: {
          claude: "rgb(80,200,120)",
          shimmer: "rgb(120,230,155)",
          merged: "rgb(80,200,120)",
        },
        blue: {
          claude: "rgb(87,140,247)",
          shimmer: "rgb(137,180,255)",
          merged: "rgb(87,140,247)",
        },
        purple: {
          claude: "rgb(178,120,255)",
          shimmer: "rgb(208,160,255)",
          merged: "rgb(178,120,255)",
        },
        pink: {
          claude: "rgb(236,116,170)",
          shimmer: "rgb(249,156,196)",
          merged: "rgb(236,116,170)",
        },
        cyan: {
          claude: "rgb(60,210,210)",
          shimmer: "rgb(100,230,230)",
          merged: "rgb(60,210,210)",
        },
      };

  const picked = palette[accent as keyof typeof palette];
  if (!picked) return baseTheme;

  return {
    ...baseTheme,
    claude: picked.claude,
    claudeShimmer: picked.shimmer,
    clawd_body: picked.claude,
    briefLabelClaude: picked.claude,
