...
function isCustomFirstPartyGateway(): boolean {
  return getAPIProvider() === "firstParty" && !isFirstPartyAnthropicBaseUrl();
}

function estimateMessageAndToolTokens(
...
export async function countMessagesTokensWithAPI(
  ...
  // Anthropic-compatible third-party gateways (like xAI via ANTHROPIC_BASE_URL)
  // commonly do not implement /messages/count_tokens. Avoid remote 404s and use
  // local rough estimation instead.
  if (isCustomFirstPartyGateway()) {
    return estimateMessageAndToolTokens(messages, tools);
  }
